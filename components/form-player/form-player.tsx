	...
	const validateCurrentQuestion = useCallback(() => {
		...
		if (answer && currentQuestion.type === "url") {
			try {
				...
				setErrors({
					...errors,
					[currentQuestion.id]: "Please enter a valid URL",
				});
				return false;
			}
		}
		...
		if (answer && currentQuestion.type === "phone") {
			...
			if (!phoneRegex.test(String(answer))) {
				setErrors({
					...errors,
					[currentQuestion.id]: "Please enter a valid phone number",
				});
				return false;
			}
		}
			...
			>
				...
				>
					...
					>
						...
						>
							<span>Made with</span>
							<span className="font-semibold">BetterForm</span>
						</a>
					</motion.div>
				</motion.div>
			</div>
		...
		>
			...
			<footer className="fixed bottom-0 left-0 right-0 p-4 flex items-center justify-between">
				...
				<a
					...
					style={{ color: theme.textColor }}
				>
					Powered by <span className="font-semibold">BetterForm</span>
				</a>
			</footer>
		</div>
	);
